...
const ChangeTimeZone = ({ p_isModalOpen, p_setIsModalOpen }) => {
  ...
  const [isModalOpen, setIsModalOpen] = useState(p_isModalOpen);
  useEffect(() => {
    setIsModalOpen(p_isModalOpen);
  }, [p_isModalOpen]);
  ...
  const showModal = () => {
    setIsModalOpen(true);
    p_setIsModalOpen(true);
  };
  const handleOk = () => {
    setIsModalOpen(false);
    p_setIsModalOpen(false);
    fetchCTZ();
  };
  const handleCancel = () => {
    setIsModalOpen(false);
  ...
  const fetchCTZ = async () => {
    let response = await fetch('http://127.0.0.1:8000/api/users/ctz', {
      method: 'POST',
      headers: {
  ...
  return (
    <>
      <Modal title="Select timezone" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
        ...
        <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
          ...
          <ul>
            {filteredTimezones.map((timezone, index) => (
              <li key={index} onClick={() => handleTimezoneClick(timezone)}>
                {timezone}
              </li>
            ))}
          </ul>
