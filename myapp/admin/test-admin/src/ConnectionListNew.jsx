...
export const ConnectionListNew = () => {
    const { id } = useParams();

    return (
        <List resource="connections" filter={{ site_id: id }} actions={<ListActions />} empty={false} > 
            <Datagrid>
                <TextField source="id" />
...
const EditConnectionButton = () => {
    ...
    return (
        <Button
            component={Link}
            to={`/sites/${connection?.site_id}/connectionsnew/${connection?.id}`}
            startIcon={<EditIcon />}
        >
            Edit
        </Button>
